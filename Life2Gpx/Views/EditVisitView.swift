                ...
                if let result = pendingSearchResult, currentCoordinate != nil {
                    ...
                    EditPlaceView(
                        place: Place(
                            ...
                            mapboxCategoryId: result.provider == .mapbox ? result.primaryCategoryId : nil,
                            foursquareVenueId: result.provider == .foursquare ? result.id : nil,
                            foursquareCategoryId: result.provider == .foursquare ? result.primaryCategoryId : nil,
                            googlePlacesId: result.provider == .google ? result.id : nil,
                            googlePlacesCategoryId: result.provider == .google ? result.primaryCategoryId : nil,
                            yelpId: result.provider == .yelp ? result.id : nil,
                            yelpCategoryId: result.provider == .yelp ? result.primaryCategoryId : nil,
                            applePlaceId: result.provider == .apple ? result.id : nil,
                            appleCategoryId: result.provider == .apple ? result.primaryCategoryId : nil,
                            osmNodeId: result.provider == .openStreetMap ? result.id : nil,
                            osmCategoryId: result.provider == .openStreetMap ? result.primaryCategoryId : nil,
                            herePlaceId: result.provider == .here ? result.id : nil,
                            hereCategoryId: result.provider == .here ? result.primaryCategoryId : nil,
                            gaodePlaceId: result.provider == .gaode ? result.id : nil,
                            gaodeCategoryId: result.provider == .gaode ? result.primaryCategoryId : nil,
            ...
            if let coordinate = currentCoordinate {
                ...
                if let waypointPlaceId = waypointPlaceId, let matchingPlace = allPlaces.first(where: { $0.placeId == waypointPlaceId }) {
                    ...
                    selectedPlace = Place(
                        ...
                        mapboxCategoryId: self.originalWaypoint?.extensions?["MapboxCategoryId"].text,
                        ...
                        googlePlacesCategoryId: self.originalWaypoint?.extensions?["GooglePlacesCategoryId"].text,
                        yelpId: self.originalWaypoint?.extensions?["YelpId"].text,
                        yelpCategoryId: self.originalWaypoint?.extensions?["YelpCategoryId"].text,
                        applePlaceId: self.originalWaypoint?.extensions?["ApplePlaceId"].text,
                        appleCategoryId: self.originalWaypoint?.extensions?["AppleCategoryId"].text,
                        osmNodeId: self.originalWaypoint?.extensions?["OsmNodeId"].text,
                        osmCategoryId: self.originalWaypoint?.extensions?["OsmCategoryId"].text,
                        herePlaceId: self.originalWaypoint?.extensions?["HerePlaceId"].text,
                        hereCategoryId: self.originalWaypoint?.extensions?["HereCategoryId"].text,
                        gaodePlaceId: self.originalWaypoint?.extensions?["GaodePlaceId"].text,
                        gaodeCategoryId: self.originalWaypoint?.extensions?["GaodeCategoryId"].text,
